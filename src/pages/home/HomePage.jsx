...
import MemePostCard from "../../components/MemePostCard/MemePostCard"; // Yeni

// JSON veri kaynaklarını import et
import allVideos from "../../data/explore.json";
import allTweets from "../../data/tweets.json"; // Hatalı yol düzeltildi
...
const JSON_BATCH_SIZE = 18; // 6'lı setlerden 3 tane (3*6=18)

const Home = () => {
  // Varsayılan görünüm 'json' olarak ayarlandı.
  const [activeView, setActiveView] = useState("json");

  // Her akış için ayrı state'ler
  ...
  // Her akış için ayrı paginasyon ve tükenme durumu
  ...
  // İlk yüklemenin yapılıp yapılmadığını kontrol etmek için ref
  const initialLoadDone = useRef({ json: false, firebase: false });

  // Görülen ID'leri yöneten fonksiyonlar
  ...
  // Firebase'den veri çeken fonksiyon
  ...
  // JSON kaynaklarından veri çeken fonksiyon
  const loadNextJsonBatch = useCallback(async () => {
      if (loading || jsonExhausted) return;
      setLoading(true);
  
      const pickRandomly = (pool, count, seenKey) => {
          ...
          for (let i = 0; i < count; i++) {
              ...
              selected.push({ ...item, id: item.id.toString(), type: seenKey === 'seenMemes' ? 'image' : (seenKey === 'seenVideos' ? 'video' : 'quote'), source: "json" });
          }
          return selected;
      };
      ...
      for(let i = 0; i < numSets; i++) {
          ...
          let shuffledSet = shuffleArray(set);
          newBatch.push(...shuffledSet);
      }
      
      if (newBatch.length < JSON_BATCH_SIZE) {
  ...
  // Sadece ilk açılışta verileri yükle
  useEffect(() => {
    if (!initialLoadDone.current.json) {
        loadNextJsonBatch();
    }
  }, [loadNextJsonBatch]);

  // View değiştiğinde, eğer o view için veri daha önce yüklenmediyse yükle
  useEffect(() => {

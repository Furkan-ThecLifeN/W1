  ...
  useEffect(() => {
    const fetchConversations = async () => {
      ...
      try {
        ...
        // Konuşmaları mapleyip diğer kullanıcı UID’sini alıyoruz
        const conversations = await Promise.all(snapshot.docs.map(async docSnap => {
          ...
          // Users koleksiyonundan diğer kullanıcı bilgilerini al
      ...
      <ul className={styles.usersMessagesBox}>
        {error ? (
          ...
          followingUsers.map(user => (
            <li key={user.uid} className={styles.userCard} onClick={() => onSelectUser(user)}>
              ...
              <div className={styles.userMessageInfo}>
                ...
                <p className={styles.lastMessage}>{user.lastMessage?.text || "Yeni mesaj yok"}</p>
              </div>
            </li>
          ))
        ) : (
          !loading && <li className={styles.statusMessage}>Henüz konuşmanız bulunmamaktadır.</li>
        )}
      </ul>
    </div>
  );
